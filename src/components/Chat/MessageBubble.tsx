...
const MessageBubble = ({ from, text }:ChatBubbleProp) => {
    ...
    return (
      <div className={`flex ${isMe ? 'justify-end' : 'justify-start'} my-2`}>
        <div className={`relative max-w-xl lg:max-w-md px-4 py-2 text-sm rounded-t-3xl ${text.length<20 ? 'break-normal':'break-all'} ${isMe ? 'bg-blue-600 text-white rounded-bl-3xl' : 'bg-gray-200 text-gray-800 rounded-br-3xl'}`}>
          {text}
